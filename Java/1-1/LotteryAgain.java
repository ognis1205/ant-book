/* $File: LotteryAgain.java, $Timestamp: Mon Mar  8 16:20:24 2021 */
import java.io.*;
import java.util.*;
import java.text.*;
import java.math.*;
...
public class LotteryAgain {
  private static FastScanner scanner;
  private static LotteryAgain solver;

  public static void main(String[] args) {
    try {
      solver = new LotteryAgain();
  ...
  private void solve() {
    ...
    int[] k = Arrays.stream(scanner.nextLine().split("\\s+")).mapToInt(e -> Integer.parseInt(e)).toArray();
    int[] kk = new int[4096];
    int p = 0;
    ...
    for (int i = 0; i < n; i++) {
      for (int j = i; j < n; j++) {
	if (Arrays.binarySearch(kk, 0, p, m - k[i] - k[j]) >= 0) {
