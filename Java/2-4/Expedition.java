/* $File: Expedition, $Timestamp: Tue Aug 10 13:08:20 2021 */
import java.io.*;
import java.nio.*;
...
public class Expedition {
  ...
  private static Expedition solver;
  ...
  public Expedition(FastScanner scan) {
    N = Integer.parseInt(scan.nextLine());
    L = Integer.parseInt(scan.nextLine());
    P = Integer.parseInt(scan.nextLine());
    A = new int[N];
    B = new int[N];
    FastScanner ds = scan.scanLine();
    for (int i = 0; i < N; i++)
      A[i] = ds.nextInt();
    FastScanner gs = scan.scanLine();
    for (int i = 0; i < N; i++)
      B[i] = gs.nextInt();
    que = new PriorityQueue<>(N, new Comparator<Integer>() {
	@Override
