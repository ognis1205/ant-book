/* $File: Frog, $Timestamp: Mon Aug  9 22:15:22 2021 */
import java.io.*;
import java.nio.*;
...
public class Frog {
  ...
  private int dp(int i) {
    if (memo.containsKey(i)) {
      ...
      int cost = 0;
      if (i == 0) cost = 0;
      else if (i == 1) cost = dp(i - 1) + Math.abs(h[i] - h[i - 1]);
      else cost = Math.min(dp(i - 1) + Math.abs(h[i] - h[i - 1]), dp(i - 2) + Math.abs(h[i] - h[i - 2]));
      memo.put(i, cost);
      return memo.get(i);
    }
  }
