/* $File: longest_common_subsequence.cc, $Timestamp: Tue Dec  1 22:51:15 2020 */
#ifdef LOCAL
#  include <algorithm>
#  include <assert.h>
#  include <bitset>
...
int Max(const int& i, const int& j) {
  ...
  if (i == n || j == m) return 0;
  if (s[i] == t[j]) return memo[{i, j}] = max(Max(i + 1, j + 1) + 1, max(Max(i + 1, j), Max(i, j + 1)));
  return memo[{i, j}] = max(Max(i + 1, j), Max(i, j + 1));
}
