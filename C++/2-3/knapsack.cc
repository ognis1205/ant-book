/* $File: knapsack.cc, $Timestamp: Mon Nov 30 21:51:25 2020 */
#ifdef LOCAL
#  include <algorithm>
#  include <assert.h>
#  include <bitset>
...
int main(int argc, char* argv[]) {
  ...
  REP (i, n) { CIN(items[i]); ASSERT(IN(items[i].fst, 1, 1e2)); ASSERT(IN(items[i].scd, 1, 1e2)); }
  CIN(W); ASSERT(IN(W, 1, 1e4));
  DBG(n, items, W);
  ...
  REP (i, n) {
    REP (j, W + 1) {
      if (j == 0) dp[i + 1][j] == 0;
      else if (j < items[i].fst) dp[i + 1][j] = dp[i][j];
      else dp[i + 1][j] = max(dp[i][j], dp[i][j - items[i].fst] + items[i].scd);
    }
  }
