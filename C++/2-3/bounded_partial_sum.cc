/* $File: bounded_partial_sum.cc, $Timestamp: Wed Dec  2 15:56:05 2020 */
#ifdef LOCAL
#  include <algorithm>
#  include <assert.h>
#  include <bitset>
...
int main(int argc, char* argv[]) {
  ...
  CINV(n, a); TRAV (e, a) ASSERT(IN(e, 1, 1e5));
  CINV(n, m); TRAV (e, m) ASSERT(IN(e, 1, 1e5));
  CIN(K); ASSERT(IN(K, 1, 1e5));
  DBG(n, K); DBG(a); DBG(m);

  vvec<int> dp; RESIZE(dp, n + 1, K + 1);
  REP (i, n) dp[i + 1][0] = m[i]; REP (i, K) dp[0][i + 1] = -1; dp[0][0] = 0;

  REP (i, n) {
    REP (j, K + 1) {
      if (dp[i][j] >= 0) dp[i + 1][j] = m[i];
      else if (j < a[i]) dp[i + 1][j] = -1;
