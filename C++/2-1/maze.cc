...
void Parse(vector<i64>* row, const i64& i, const i64& j, char* c) {
  switch (*c) {
  case start:
    coord_s = {i, j};
    row->emplace_back(0);
  ...
  case goal:
    coord_g = {i, j};
    row->emplace_back(kInf);
    return;
  default:
    row->emplace_back(*c == wall ? -1 : kInf);
    return;
  }
}
...
int main(int argc, char* argv[]) {
  ...
  REP (i, N) {
    vector<i64> row;
    cin >> SplitAs<char>([&](const i64& j, char* c) { Parse(&row, i, j, c); });
